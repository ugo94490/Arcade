...
class QixPlayer : public IGameObject {
    public:
        ...
        static std::list<std::shared_ptr<QixGround>> getzone(std::list<std::shared_ptr<QixGround>> tiles);
        static char check_type_on_pos(std::pair<float, float> const &poscheck, std::list<std::shared_ptr<QixGround>> const &tiles);
        static std::pair<float, float> getPosDirection(std::pair<float, float> basepos, int direction);
        static std::shared_ptr<QixGround> getGroundOnPos(std::pair<float, float> pos, std::list<std::shared_ptr<QixGround>>);
        static int check_qix_here(std::list<std::shared_ptr<QixGround>> const &zone, std::shared_ptr<QixQix> const &qix);
